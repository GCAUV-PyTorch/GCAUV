...
from functional import degree_drop_weights, pr_drop_weights, evc_drop_weights, feature_drop_weights, \
    closeness_drop_weights, closeness_centrality, betweenness_centrality, betweenness_drop_weights
from model import GCAUV
    ...
    for epoch in loop:
        ...
        for idx, graphs in enumerate(train_loader):
            ...
            if args.drop_scheme == 'degree':
                drop_weights1 = degree_drop_weights(views1.edge_index).to(device)
                drop_weights2 = degree_drop_weights(views2.edge_index).to(device)
            elif args.drop_scheme == 'pr':
                drop_weights1 = pr_drop_weights(views1.edge_index, aggr='sink', k=200).to(device)
                drop_weights2 = pr_drop_weights(views2.edge_index, aggr='sink', k=200).to(device)
            elif args.drop_scheme == 'evc':
                drop_weights1 = evc_drop_weights(views1).to(device)
                drop_weights2 = evc_drop_weights(views2).to(device)

            elif args.drop_scheme == 'closeness':
                drop_weights1 = closeness_drop_weights(views1).to(device)
                drop_weights2 = closeness_drop_weights(views2).to(device)
            elif args.drop_scheme == 'betweenness':
                drop_weights1 = betweenness_drop_weights(views1).to(device)
                drop_weights2 = betweenness_drop_weights(views2).to(device)
            else:
                drop_weights1 = None
                drop_weights2 = None
            ...
            drop = DropEdgeAndFeature(drop_weight=drop_weights1, d_fea=args.drop_feature_rate_1, d_edge=args.drop_edge_rate_1)
            views1 = drop.drop_fea_edge(views1)
            #drop.set_drop_ratio(d_fea=args.drop_feature_rate_2, d_edge=args.drop_edge_rate_2)
            drop = DropEdgeAndFeature(drop_weight=drop_weights2, d_fea=args.drop_feature_rate_2, d_edge=args.drop_edge_rate_2)
