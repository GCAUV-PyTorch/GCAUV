...
class GCAUV(torch.nn.Module):
    ...
    def _batched_semi_emd_loss(self, out1, avg_out1, out2, avg_out2, lamb=20, rescale_ratio=None):
        ...
        cost_matrix = 1-self.sim(out1, out2)#原始成本矩阵
        if rescale_ratio is not None:
            cost_matrix = cost_matrix * rescale_ratio#考虑拓扑距离的成本矩阵
    ...
    def batched_semi_emd_loss(self, reps1, reps2, batch1, batch2, original_idx1=None, original_idx2=None):
        ...
        if original_idx1 != None and original_idx2 != None:
            ...
            rescale_ratio = torch.sigmoid(topology_dist/self.topo_t)#拓扑距离的缩放因子
            topo_mask = torch.bmm(idx_mask1[:,:,None].float(), idx_mask2[:,None,:].float()).bool()
            loss_pos = self._batched_semi_emd_loss(out1, avg_out1, out2, avg_out2, rescale_ratio=rescale_ratio) * 2#正样本
        else:
        ...
        for i in range((batch_size-1)):
            ...
            total_neg_loss += f(self._batched_semi_emd_loss(out1, avg_out1, out1_perm, avg_out1_perm)) + f(self._batched_semi_emd_loss(out1, avg_out1, out2_perm, avg_out2_perm))#负样本
...
class GAT(torch.nn.Module):
    def __init__(self,input_dim, layer_num=2, hidden=128):
        ...
        self.convs = ModuleList()
        if self.layer_num > 1:
            self.convs.append(GATConv(in_channels=input_dim, out_channels=hidden * 2,heads=8,concat=False))
            for i in range(layer_num - 2):
                self.convs.append(GATConv(in_channels=hidden * 2, out_channels=hidden * 2))
                glorot(self.convs[i].weight)
            self.convs.append(GATConv(in_channels=hidden * 2, out_channels=hidden))
            # glorot(self.convs[-1].weight)

        else:  # one layer gcn
            self.convs.append(GATConv(in_channels=input_dim, out_channels=hidden))
            glorot(self.convs[-1].weight)
    def forward(self,data):
        x, edge_index = data.x, data.edge_index
        for i in range(self.layer_num):
...
class MLP(torch.nn.Module):
    def __init__(self,input_dim,layer_num,hidden):
        super(MLP, self).__init__()
        self.main = torch.nn.Sequential(
            torch.nn.Linear(input_dim, hidden*2),
